...
class StaticFileRoute(object):
    ...
    def __call__(self, *args):
        ...
        ctx.response.contet_type = mimetypes.types_map.get(fext.lower(), 'application/octet-stream')
        ...
        if not hasattr(self, '_headers'):
            ...
            for k, v in self._environ.iteritems():
                if k.startswith('HTTP_'):
                    hdrs[k[5:].replace('_', '_').upper()] = v.decode('utf-8')
            self._headers = hdrs
        return self._headers
    ...
    def status(self, value):
        if isinstance(value, (int, long)):
            ...
            if _RE_RESPONSE_STATUS.match(value):
                ...
                raise ValueError('Bad respone code: %s' % value)
        else:
...
def _default_error_handler(e, start_response, is_debug):
    ...
    if is_debug:
        return _debug()
    return ('<html><body><h1>500 Internal Servel Error</h1><h3>%s</h3></body></html>' % str(e))
...
class WSGIApplication(object):
    ...
    def add_interceptor(self, func):
        self._check_not_runnig()
        self._interceptors.append(func)
        logging.info('Add interceptor: %s' % str(func))

    def run(self, port=9000, host='127.0.0.1'):
        from wsgiref.simple_server import make_server
        logging.info('application (%s) will start ad %s: %s...' % (self._document_root, host, port))
    ...
    def get_wsgi_application(self, debug=False):
        ...
        def wsgi(env, start_response):
            ...
            try:
                ...
                response.set_header('Location', e.headers)
